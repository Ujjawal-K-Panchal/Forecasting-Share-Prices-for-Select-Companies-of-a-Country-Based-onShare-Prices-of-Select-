...
#ohe = OneHotEncoder()
dataset = pd.read_csv('unbiaseddataset.csv')
X = dataset.iloc[:,:134+1].values
Y = dataset.iloc[:,136].values#Company for which analysis is to be done +1
#X = X[:,68:]
'''
...
rfc = RandomForestClassifier(n_estimators = 20, random_state = 0, max_features = 95 , class_weight='balanced') # 15 by experimentation.
rfc.fit(X_train, Y_train)

#predicting
Y_pred = rfc.predict(X_test) # X_test is already transformed
